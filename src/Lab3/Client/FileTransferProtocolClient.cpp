...
int FileTransferProtocolClient::send(string filename, istream &file, size_t file_size) {
	...
	StreamSocketSender sender(SUGGESTED_STREAM_CHUNK_SIZE); //Send chunks of pre-determined size
...
int FileTransferProtocolClient::receive(string filename, ostream &file) {
	...
	StreamSocketReceiver receiver(SUGGESTED_STREAM_CHUNK_SIZE); //Receive in chunks of pre-determined size
